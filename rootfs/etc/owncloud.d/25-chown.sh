...
then
  ...
  find ${OWNCLOUD_PRE_INSTALL_PATH} -iname *.sh | xargs -r -0 chmod +x
  find ${OWNCLOUD_POST_INSTALL_PATH} -iname *.sh | xargs -r -0 chmod +x
  find ${OWNCLOUD_PRE_SERVER_PATH} -iname *.sh | xargs -r -0 chmod +x
  find ${OWNCLOUD_POST_SERVER_PATH} -iname *.sh | xargs -r -0 chmod +x
